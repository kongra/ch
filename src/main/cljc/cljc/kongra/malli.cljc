...
#?(:clj
   (def NonBlank
     [:fn
      {:error/message "should be a non-blank string"}
      (fn [s] (and (string? s) (not (blank? s))))]))
...
#?(:clj (def Person? (m/validator Person)))

#_(:clj
   (let [person {:name "aaa" :age 10}]
     (quick-bench (Person? person))))
